...
function SandboxDriver() {
  ...
  // Make RealtimeBus an EventEmitter.
  var emitter = new EventEmitter();
  _.extend(this.svclib.RealtimeBus, emitter);
}
...
SandboxDriver.prototype.sendEvent = function(evtName, evtData) {
  if (!_.isString(evtName)) {
    throw new Error('sendEvent() requires a string as the event name.');
  }
