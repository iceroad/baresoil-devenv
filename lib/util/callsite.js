...
var LOG_ROOT = path.resolve(path.join(__dirname, '../../'));
...
module.exports = function getCallsite(stack, idx) {
  ...
  var stackLocations = _.filter(_.map(stack, function(stackLineStr) {
    stackLineStr = stackLineStr.replace(/^\s+|\s+$/mgi, '');
    var groups = stackLineStr.match(/\((\S+):(\d+):(\d+)\)$/);
    if (!groups) groups = stackLineStr.match(/at (.+):(\d+):(\d+)$/);
    if (groups && groups.length === 4) {
      var absPath = groups[1], relPath;
